...
class AppStore extends Component {
  ...
  getFilterAppItems = () => {
    ...
    const filterAppItems = appsList.filter(
  ...
  onSearchAppName = event => {
    const {searchInput} = this.state
    this.setState({searchInput: event.target.value.toLowerCase()})
  }

  render() {
    const filterAppItems = this.getFilterAppItems()
    const {activeTabId, searchInput} = this.state
    return (
      <div className="app-container">
        <h1 className="app-store">App Store</h1>
        <div className="input-container">
          <input type="search" onChange={this.onSearchAppName} />
          <img
        ...
        <ul className="app-container-list">
          {filterAppItems
            .filter(app => app.appName.toLowerCase().includes(searchInput))
            .map(app => (
              <AppItem details={app} key={app.appId} />
            ))}
        </ul>
      </div>
    )
